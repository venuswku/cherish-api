...
// Posts a suggested act of kindness, which will later be reviewed/approved by me.
router.route("/suggest").post((req, res) => {
  ...
  let actionData = {
    ...
    for: suggestedFor,
    likes: (suggesterLikes ? 1 : 0),
    done: (suggesterDidAction ? 1 : 0),
    approved: false,
  };
  if (suggester !== undefined) {
    actionData[suggestedBy] = suggester;
  }
  if (suggestedImageLink !== undefined) {
    actionData[image] = suggestedImageLink;
  }
...
// Route: /actions/
// Gets all actions from the MongoDB Atlas database.
...
// Route: /actions/:id
// Gets an action with the specified object id, which is automatically generated by MongoDB.
